    ...
    */
    ...
    public R add(@RequestBody ShujiOrderEntity shujiOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,shujiOrder:{}",this.getClass().getName(),shujiOrder.toString());
            ...
            else if(shujiEntity.getShujiNewMoney() == null){
                return R.error(511,"书籍价格不能为空");
            }
            ...
            double balance = yonghuEntity.getNewMoney() - shujiEntity.getShujiNewMoney()*1;//余额
            if(balance<0)
    ...
    public R huanshu(@RequestParam("id") Integer id, HttpServletRequest request){
        ...
        yonghuEntity.setNewMoney(yonghuEntity.getNewMoney()+shujiEntity.getShujiNewMoney());
        yonghuService.updateById(yonghuEntity);
        shujiService.updateById(shujiEntity);
